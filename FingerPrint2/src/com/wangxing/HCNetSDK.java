...
//NET_DVR_Login()参数结构
    ...
    public static class NET_DVR_FINGER_PRINT_INFO_COND extends Structure
    {
        ...
        public byte[] byRes1 = new byte[26]; //保留
    ...
    public static class NET_DVR_FINGER_PRINT_CFG extends Structure
    {
        ...
        public byte[] byEnableCardReader = new byte[MAX_CARD_READER_NUM_512];   //需要下发指纹的读卡器，数组下标表示读卡器序号，数组值：0- 不下发，1- 下发
        public byte byFingerPrintID;    //指纹编号，有效值范围为1~10
        public byte byFingerType;   //指纹类型：0- 普通指纹，1- 胁迫指纹
        public byte[] byRes1 = new byte[30];    //保留，置为0
        public byte[] byFingerData = new byte[MAX_FINGER_PRINT_LEN];    //指纹数据内容
        public byte[] byRes = new byte[64]; //保留，置为0
    }

    public static final int NET_SDK_CALLBACK_STATUS_SUCCESS = 1000; //查找成功
    public static final int NET_SDK_CALLBACK_STATUS_PROCESSING = 1001; //查找中,回调进度值(暂不支持)
    public static final int NET_SDK_CALLBACK_STATUS_FAILED = 1001; //查找失败
    // dwStatus为NET_SDK_CALLBACK_STATUS_PROCESSING，lpBuffer：4字节状态 + 32字节卡号；
    public static  class NET_DVR_FINGER_PRINT_CFG_PROCESSING extends Structure
    {
        public int dwstatus;  //结构体大小
        public byte[] byCardNo = new byte[ACS_CARD_NO_LEN]; //指纹关联的卡号
    }

    // dwStatus为NET_SDK_CALLBACK_STATUS_FAILED，lpBuffer：4字节状态 + 4字节错误码 + 32字节卡号
    public static class NET_DVR_FINGER_PRINT_CFG_FAILED extends Structure
    {
        public int dwstatus;  //结构体大小
        public int dwErrorCode;  //结构体大小
        public byte[] byCardNo = new byte[ACS_CARD_NO_LEN]; //指纹关联的卡号
    }

    //FingerPrint2
    public static final int NET_DVR_GET_FINGERPRINT = 2563; //获取指纹
    ...
    {
        public int dwSize;
        public int dwFingerprintNum;    //指纹数量（指纹获取或下发时有效，指纹获取时为 0xffffffff 表示获取所有指纹信息）
        public byte[] byCardNo = new byte[ACS_CARD_NO_LEN]; //指纹关联的卡号（指纹获取时有效）
        public int dwEnableReaderNo;    //指纹读卡器编号（指纹获取时有效）
        public byte byFingerPrintID;    //指纹编号，有效值范围 1-10（指纹获取时有效，0xff 表示该卡 所有指纹）
        public byte[] byRes = new byte[131];
    }

    // NET_DVR_GetNextRemoteConfig函数的NET_DVR_FINGERPRINT_RECORD参数定义
